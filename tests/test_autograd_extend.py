...
class LinearFunction(Function):
    ...
    def forward(input, weight, bias=None):
        ...
        print(torch.cuda.memory_reserved(0) / 1024**3, output.numel() * output.element_size() / 1024**3)
        if bias is not None:
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            grad_input = LinearFunction.submit_to_ray(grad_output.view(-1, grad_output.shape[-1]), weight.t())
        if ctx.needs_input_grad[1]:
            ...
            grad_weight = LinearFunction.submit_to_ray(grad_output.t(), input).t()
    ...
    def submit_to_ray(A, B):
        ...
        size_each_actor = B.shape[1] // len(gpu_actors)
